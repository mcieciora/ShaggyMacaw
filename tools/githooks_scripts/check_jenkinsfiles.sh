...
find . -iname "*jenkinsfile" > "$JENKINS_FILE_CHECK_FILE"
...
cat < "$JENKINS_FILE_CHECK_FILE" | xargs -I {} curl --user "jenkins_user:pass" -X POST -F "jenkinsfile=<{}" "$JENKINS_SERVER_HOST"/pipeline-model-converter/validate > "$CURL_LOG_FILE"
