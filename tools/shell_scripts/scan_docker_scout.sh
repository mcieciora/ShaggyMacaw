...
for TAG in $TAGS; do
  ...
  if [ "$SCAN_RESULT" -ne 0 ]; then
    ...
    docker run --rm -e DOCKER_SCOUT_HUB_USER="$USERNAME" -e DOCKER_SCOUT_HUB_PASSWORD="$PASSWORD" -v /var/run/docker.sock:/var/run/docker.sock docker/scout-cli recommendations $DOCKERHUB_REPO:"$TAG" > scan_scout_"$TAG".txt
