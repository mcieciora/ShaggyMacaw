...
def test__unittest__fen__generate_board_squares__squares_order():
    test_fen = "rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1"
    test_data = Fen(test_fen)
    assert test_data.board_squares[0] == "a1", f"Expected: a1, actual: {test_data.board_squares[0]}"
    assert test_data.board_squares[63] == "h8", f"Expected: h8, actual: {test_data.board_squares[0]}"
