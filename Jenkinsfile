            ...
            parallel {
                stage ("Push docker image") {
                    ...
                    steps {
                        script {
                            docker.withRegistry("", "dockerhub_id") {
                                def customImage = docker.build("${DOCKERHUB_REPO}:${env.BRANCH_NAME}-${curDate}")
                                customImage.push()
                                if (env.BRANCH_NAME == "master") {
                                    customImage.push("latest")
                                }
                            }
                            sh "docker rmi ${DOCKERHUB_REPO}:${env.BRANCH_NAME}-${curDate}"
                        }
                    }
                }
                stage ("Push tag") {
                    ...
                    steps {
                        script {
                            def TAG_NAME = "${env.BRANCH_NAME}-${curDate}"
                            withCredentials([sshUserPrivateKey(credentialsId: "github_id", keyFileVariable: 'key')]) {
                                sh 'GIT_SSH_COMMAND="ssh -i $key"'
